...
function router (bits) {
  ...
  for (var c = 0; c < out.length; ++c) {
    stateMachine = stateMachine(out[c], accMethod, accRoute, accTarget, routes)
  }
...
function _methodMachine (ch, method, route, target, store) {
  ...
  if (ws(ch)) {
    if (!/(\*|GET|PUT|POST|DELETE)/.test(method.join(''))) {
      throw new Error(
        'expected GET, PUT, POST, DELETE, or * — got ' + method.join('')
      )
    }
