...
module.exports = class Router {
  ...
  enumerateMatch (method, route) {
    const iter = enumerateMatch(this, method, normalize(route), null)
    var {value, done} = iter.next()
    while (!done) {
      const context = value[0].match(value[1])
      const next = iter.next(context || null)
      value = next.value
      done = next.done
    }
    return value
  ...
  * enumerate (method, route) {
    const iter = enumerateMatch(this, method, normalize(route), null)
    var {value, done} = iter.next()
    while (!done) {
      yield [...value[2], value[0]]
      const context = value[0].match(value[1])
      const next = iter.next(context || null)
      value = next.value
      done = next.done
...
function * enumerateMatch (router, method, route, lastMatch, targets = []) {
  for (const target of router) {
    if (!target.accepts(method)) {
      continue
    }
    const context = yield [target, route, targets]
    if (!context) {
      continue
    }
    ...
    if (value && value[IS_ROUTER_SYM]) {
      ...
      const result = yield * enumerateMatch(
        value,
        method,
